...
function CasesTable(props) {
	...
	async function handleDeselect() {
		const resp = await axios.post(Backend.URL + '/delete_case', {ids: selected}, { withCredentials: true, headers: {"Access-Control-Allow-Origin": "*"} });
		if (resp.data.ids) {
			setLoading(true);
			init();
			setLoading(false);
	...
	function handleClick(item) {
		props.history.push(`/admin/cases/items/${item.id}`);
	}
							...
							.map(n => {
								...
								return (
									...
									>
										...
										<TableCell
											className="w-40 p-8 text-center"
											component="th"
											scope="row"
										>
											{n.thumbnailURL ? (
												<img
													...
													src={Backend.URL + `/cases/` + n.thumbnailURL}
													alt={n.subject}
												/>
											) : (
												<img
